...
笏懌楳笏 backend/
笏・  笏懌楳笏 main.py                                # FastAPI 繧ｨ繝ｳ繝医Μ繝昴う繝ｳ繝・(WebSocket + REST API) 窶ｻ2026-04-22 14:57 蜀崎ｵｷ蜍募ｮ御ｺ・(PID: 2628)
...
---
最終更新: 2026-04-22 15:00


### 25. 会話履歴の集中管理とグローバル表示 (2026-04-22)

**(a) 当初の設計**: 会話履歴およびシステムアーティファクト（実装計画等）は、ワークスペースの場所に関わらず ~/.gemini/antigravity/brain に一括保存される設計。
**(b) 課題とユーザーフィードバック**: ユーザーより「プロジェクトごとに履歴が紐付いていると思っていたが、別のプロジェクトの履歴が表示される」という指摘を受けた。また、履歴が一部しか表示されないため消失したように見える。
**(c) 採用した方針**: 物理的なデータは rain ディレクトリ内に保持されており、安全であることを確認。現状はUI仕様としてグローバル表示であることを説明し、今後の改善検討事項とした。
