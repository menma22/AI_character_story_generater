...
├── backend/
│   ├── main.py                                # FastAPI エントリポイント (WebSocket + REST API) ※2026-04-20 22:34 再起動完了 (PID: 19164)
...
│           │       ├── 001_full.json          # 全行動ログ
│           │       ├── 002_summary.json       # 要約
│           │       └── 003_summary.json       # 段階的忘却
│           ├── diaries/day_NN.md              # 日記本文（独立DB）
│           └── backups/                       # 日記再生成時の過去データ退避先（バージョン管理）
│               └── {session_id}/              # 再生成セッションごとのアーカイブ
├── frontend/
...
### Stage 45: 日記再生成機能の追加と実行状態のクリーンアップ

**(a) 原案設計**: 日記の再生成は、セクション選択モーダルから選択可能だったが、バックエンドの `DailyLoopOrchestrator` が「最新の保存済みの日」を基準に開始するため、既に7日分生成されていると何も処理されずに終了していた。また、日記表示エリアに直接再生成ボタンがなかった。

**(b) 変更点・根拠**: ユーザーが「日記をやり直したい」と意図した場合、既存の進行状態（Day 1-7の記憶やムード推移）を一度破棄して Day 1 から生成し直す必要がある。また、UI上で日記を確認しながらその場で再生成できる方がUXとして優れている。

**(c) 採用したベストプラクティス**:
- `CharacterPackage` に `diaries` フィールドを追加し、データをパッケージ内に明示的に保持
- 再生成開始前に `daily_logs`, `mood_states`, `short_term_memory`, `key_memories` の各ディレクトリを削除し、フルリセットを保証
- フロントエンドの日記タブに `artifactActionBar` を統合し、直感的な再操作を可能にする
