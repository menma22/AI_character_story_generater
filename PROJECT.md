...
├── backend/
│   ├── main.py                                # FastAPI エントリポイント (WebSocket + REST API) ※2026-04-12 21:50 再起動完了 (PID 79184)
...
- **(c) 採用したベストプラクティス**:
  - **最新の状態**: プロセス再起動完了 (2026-04-12 21:50)
  - **詳細**: ポート 8001 を使用していたプロセス (PID 77692) を停止し、`python -m backend.main` により新規プロセス (PID 79184) を起動。動作確認済み。
