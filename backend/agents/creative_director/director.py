...
7. 7日間の物語で何が起きるかの骨格を示せ。山場があった方がいい場合は山場を設計してください。
    ...
    async def run(self, theme: Optional[str] = None) -> ConceptPackage:
        ...
        async def request_critique(concept_package: dict = None) -> dict:
            ...
            if search_count < min_searches:
                return {"status": "BLOCKED", "message": f"ERROR: Web検索が{search_count}回しか実行されていません。最低{min_searches}回のsearch_webを実行してからrequest_critiqueを呼び出してください。残り{min_searches - search_count}回の検索が必要です。"}
        ...
        async def self_reflect(concept_package: dict = None) -> dict:
            ...
            if not critique_passed:
                return {"status": "BLOCKED", "message": "ERROR: 先にrequest_critiqueでpassを得てください。critique未passの状態でself_reflectは実行できません。"}
            ...
            if convinced:
                ...
                self_reflect_convinced = False
                critique_passed = False  # リセット → 再ドラフトへ
                ...
                return {"status": "FAILED", "message": f"まだ確信が持てません。理由: {reason}。改善案: {suggestion}。ドラフトを改善し、再度request_critique → self_reflectの順で進めてください。"}
        ...
        async def submit_final_concept(concept_package: dict = None) -> dict:
            ...
            if not critique_passed or not self_reflect_convinced:
                ...
                return {"status": "BLOCKED", "message": f"ERROR: 提出がブロックされました。理由: {', '.join(blocked_reasons)}。critique→self_reflectの順で両方passしてからsubmitしてください。"}
        ...
        tools = [
            ...
            AgentTool(
                name="search_web",
                description=f"指定したキーワードでWeb検索を行い、記事や情報を取得します。キャラクター設定や物語のネタ集め、面白い設定の調査など、執筆前のリサーチに必ず使用してください。最低{min_searches}回の呼び出しが必須です。",
                input_schema={
                    "type": "object",
                    "properties": {
            ...
            AgentTool(
                ...
                description=f"現在のconcept_packageドラフトを厳しく評価し、面白さや心理学的深さ、Redemption Biasの有無を判定してもらいます。search_webを最低{min_searches}回実行済みでないとBLOCKEDされます。concept_packageはJSON文字列として渡してください。",
                input_schema={
                    "type": "object",
                    "properties": {
            ...
            AgentTool(
                name="self_reflect",
                description="request_critiqueでpassを得た後に呼び出す内省ツール。「本当にこれでいいのか？妥協していないか？」を自分自身に問い、確信が持てるか判定します。確信が持てない場合はcritique_passedもリセットされ、再ドラフトが必要になります。concept_packageはJSON文字列として渡してください。",
                input_schema={
                    "type": "object",
                    "properties": {
                        "concept_package_json": {"type": "string", "description": "現在のドラフトデータ全体のJSON文字列"}
            ...
            AgentTool(
                ...
                description="request_critiqueでpass かつ self_reflectでconvinced=trueを得た後にのみ呼び出せます。最終的なconcept_packageをシステムに提出し、タスクを終了します。両方の品質ゲートをpassしていない場合はBLOCKEDされます。",
                input_schema={
                    "type": "object",
                    "properties": {
...
あなたは以下の5フェーズを順番に実行してください。ショートカットは許可されていません。

【Phase 1: 計画】
- まず、どのような方向性のキャラクターを作るか、短い計画メモを内部で考える
- 検索すべきキーワードを3-5個リストアップする
- 「何を知りたいか」「何を調べれば面白くなるか」を明確にする

【Phase 2: リサーチ（search_webを最低{min_searches}回必須）】
- search_webを最低{min_searches}回呼び出し、多角的な情報を収集する
- 検索の例:
  ① キャラクターの職業・立場に関するリアルな情報（実態、あるあるエピソード）
  ② 心理学的な特性や興味深い人間の行動パターン
  ③ 類似テーマの創作作品・実話のユニークな設定
- 各検索結果から使えるアイデアを整理する
- {min_searches}回未満の検索ではrequest_critiqueツールが拒否されます

【Phase 3: ドラフト → 外部批評】
- 収集した情報を統合してconcept_packageのドラフトを作成する
- request_critiqueツールでドラフトを客観的に評価する
- verdict が "refine" の場合は指摘に基づいて改善し、再度request_critique

【Phase 4: 自己内省 — 「本当にこれでいいのか？」】
- request_critiqueでpassを得たら、self_reflectツールを呼び出す
- 「本当にこのキャラは面白いか？読みたいか？妥協していないか？」を正直に自問
- convinced=falseが返った場合: critique_passedもリセットされる。改善してPhase 3からやり直す
- convinced=trueが返るまで、絶対に妥協しない

【Phase 5: 確信を持って提出】
- critique_passed=True かつ self_reflect_convinced=True の状態でのみ
- submit_final_conceptツールで最終データを提出する
"""
