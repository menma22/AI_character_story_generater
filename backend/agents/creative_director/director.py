...
from backend.models.character import ConceptPackage, StoryCompositionPreferences
...
[G] ユーザー構成方針との整合性（構成方針が指定されている場合のみ評価）:
  □ 指定された物語構造に沿ったstory_outlineになっているか
  □ 指定された感情トーンとgenre_and_world/narrative_themeが矛盾しないか
  □ 指定されたキャラクターアーク類型とpsychological_hintsの方向性が一致しているか
  □ 指定されたクライマックス構造・ペーシングがstory_outlineに反映されているか
  □ 指定された語り口がキャラクター設計と調和しているか
...
出力形式:
{
  "checks": {
    ...
    "G_user_preferences": {"passed": true/false, "comment": "..."}
  },
...
class CreativeDirector:
    ...
    def __init__(self, profile: EvaluationProfile, ws_manager=None, regeneration_context: Optional[str] = None, api_keys: Optional[dict] = None, composition_preferences: Optional[StoryCompositionPreferences] = None):
    ...
    async def run(self, theme: Optional[str] = None) -> ConceptPackage:
        ...
        # Human in the Loop: 構成プリファレンスの注入
        if self.composition_preferences:
            prefs = self.composition_preferences
            pref_lines = []
            label_map = {
                "narrative_structure": ("物語構造", {
                    "three_act": "三幕構成", "kishotenketsu": "起承転結", "heros_journey": "英雄の旅",
                    "story_circle": "ストーリーサークル", "freytag": "フライタークのピラミッド",
                    "save_the_cat": "Save the Cat", "fichtean": "フィヒテ曲線",
                    "in_medias_res": "イン・メディアス・レス", "circular": "円環構造",
                    "episodic": "エピソード構造", "parallel": "並行構造", "nonlinear": "ノンリニア構造",
                }),
                "emotional_tone": ("感情トーン", {
                    "heartwarming": "ほのぼの・心温まる", "bittersweet": "ほろ苦い・ビタースウィート",
                    "comedic": "コミカル・ユーモラス", "tragic": "悲劇的", "melancholic": "メランコリック",
                    "suspenseful": "サスペンスフル", "nostalgic": "ノスタルジック", "serene": "穏やか・平和",
                    "hopeful": "希望に満ちた", "eerie": "不穏・不安", "passionate": "情熱的",
                    "ironic": "皮肉・ドライ",
                }),
                "character_arc": ("キャラクターアーク", {
                    "positive_change": "ポジティブチェンジ（嘘→真実）", "flat": "フラット（信念が世界を変える）",
                    "disillusionment": "幻滅（苦い真実の発見）", "fall": "堕落（嘘にしがみつく）",
                    "corruption": "腐敗（善→悪）", "coming_of_age": "成長（未熟→成熟）",
                    "redemption": "贖罪（過ち→償い）", "transformation": "変容（根本的アイデンティティ変化）",
                }),
                "theme_weight": ("テーマの重さ", {
                    "escapist": "ライト・エスケーピスト", "slice_of_life": "スライス・オブ・ライフ",
                    "emotional": "感情探索", "social": "社会的観察", "psychological": "心理的深層",
                    "philosophical": "哲学的・実存的", "spiritual": "スピリチュアル・超越的",
                    "allegorical": "寓話的・象徴的",
                }),
                "climax_structure": ("クライマックス構造", {
                    "single": "単一クライマックス", "multiple_peaks": "複数ピーク",
                    "gradual": "漸進的上昇", "wave": "波状構造", "back_loaded": "後半集中型",
                    "anti_climax": "アンチクライマックス", "front_loaded": "初頭衝撃型",
                    "symmetric": "対称構造",
                }),
                "genre": ("ジャンル", {
                    "coming_of_age": "成長物語", "healing": "癒し・回復", "workplace": "職場ドラマ",
                    "mystery": "ミステリー", "romance": "ロマンス", "family": "家族ドラマ",
                    "friendship": "友情・仲間", "adventure": "冒険・探索",
                    "magical_realism": "日常ファンタジー", "introspection": "自己探求・内省",
                    "seasonal": "季節・移ろい", "survival": "サバイバル・克服",
                }),
                "pacing": ("ペーシング", {
                    "slow_burn": "スローバーン", "fast_paced": "ハイテンポ",
                    "rhythmic": "リズミック交替", "pulse": "パルスペーシング",
                    "accelerating": "加速型", "decelerating": "減速型",
                    "even": "均等配分", "breath": "ブレス型（間を活かす）",
                }),
                "narrative_voice": ("語り口", {
                    "introspective": "内省的", "observational": "観察的",
                    "emotional": "感情的・率直", "detached": "客観的・距離を置いた",
                    "stream_of_consciousness": "意識の流れ", "humorous": "ユーモラス・自虐的",
                    "poetic": "詩的・抒情的", "conversational": "対話的・語りかけ",
                    "fragmentary": "断片的・メモ的", "unreliable": "信頼できない語り手",
                }),
            }
            for field_name, (label, value_map) in label_map.items():
                value = getattr(prefs, field_name, None)
                if value:
                    display = value_map.get(value, value)
                    pref_lines.append(f"- {label}: {display}")
            if prefs.free_notes:
                pref_lines.append(f"- 自由記述: {prefs.free_notes}")

            if pref_lines:
                prefs_block = "\n".join(pref_lines)
                user_msg += f"\n\n【ユーザー指定の構成方針（Human in the Loop）】\n{prefs_block}\n\n上記の構成方針を尊重してconcept_packageを設計してください。未指定の項目はあなたの判断で最適な選択をしてください。"
