...
【出力形式】JSON:
{
  "speech_characteristics": {
    "concrete_features": {
      ...
      "catchphrases": ["実際に口にするフレーズ1", "フレーズ2", "フレーズ3"],
    ...
    "abstract_feel": "この人の喋り方を一言で表現するなら（例：『壊れかけのラジオのように途切れ途切れで、でも確かに何かを伝えようとしている』『春風のように柔らかいが、芯には冷たい鉄がある』）",
...
class PhaseA1Orchestrator:
    ...
    async def run(self) -> PhaseA1Result:
        ...
        all_context = (
            f"concept_package:\n{concept_json}\n\n"
            f"basic_info:\n{basic_json}\n\n"
            f"social_position:\n{json.dumps(social_obj.model_dump(mode='json'), ensure_ascii=False)}\n\n"
            f"family_and_intimacy:\n{json.dumps(family_obj.model_dump(mode='json'), ensure_ascii=False)}\n\n"
            f"current_life_outline:\n{json.dumps(lifestyle_obj.model_dump(mode='json'), ensure_ascii=False)}\n\n"
            f"dream_timeline:\n{json.dumps(dream_obj.model_dump(mode='json'), ensure_ascii=False)}\n\n"
            f"values_core:\n{json.dumps(values_obj.model_dump(mode='json'), ensure_ascii=False)}\n\n"
            f"secrets:\n{json.dumps(secret_obj.model_dump(mode='json'), ensure_ascii=False)}\n\n"
            f"relationship_network:\n{json.dumps(rel_net_obj.model_dump(mode='json'), ensure_ascii=False)}"
        )
