...
class PhaseA3Orchestrator:
    ...
    async def run(self) -> AutobiographicalEpisodes:
        ...
        async def draft_episodes(episodes_json: str = None, **kw) -> dict:
            ...
            # 基本バリデーション
            if len(episodes) < 5:
                issues.append(f"エピソード数が{len(episodes)}個です。最低5個必要です。")
            if len(episodes) > 8:
                issues.append(f"エピソード数が{len(episodes)}個です。最大8個です。")

            # McAdamsカテゴリ分布チェック
            categories = [ep.get("metadata", {}).get("category", "") for ep in episodes]
            redemption_count = categories.count("redemption")
            if redemption_count > 2:
                issues.append(f"redemptionが{redemption_count}個あります（最大2個）。")
            if "contamination" not in categories:
                issues.append("contaminationカテゴリが0個です（最低1個必要）。")
            if "loss" not in categories:
                issues.append("lossカテゴリが0個です（最低1個必要）。")
            if "ambivalent" not in categories:
                issues.append("ambivalentカテゴリが0個です（最低1個必要）。")
            if "dream_origin" not in categories:
                issues.append("dream_originカテゴリが0個です（1個必要）。")

            # narrative長チェック
            for i, ep in enumerate(episodes):
                narrative = ep.get("narrative", "")
                if len(narrative) < 100:
                    issues.append(f"ep_{i+1}のnarrativeが{len(narrative)}字です（最低200字推奨）。")

            await self._notify(f"ドラフト受領: {len(episodes)}個のエピソード")

            if issues:
                return {"status": "NEEDS_FIX", "message": f"構造的な問題があります: {'; '.join(issues)}。修正してから再度draft_episodesを呼び出してください。"}
            else:
                return {"status": "SUCCESS", "message": f"{len(episodes)}個のエピソードを受領しました。request_critiqueで品質評価を受けてください。"}
        ...
        async def request_critique(episodes_json: str = None, **kw) -> dict:
            ...
            critique_result = await call_llm(
                ...
                user_message=f"以下のキャラクターコンテキストとエピソード群を評価してください:\n\n--- キャラクター ---\n{context}\n\n--- エピソード ---\n{json.dumps(data, ensure_ascii=False, indent=2)}",
                json_mode=True,
                cache_system=True,
            )
        ...
        async def self_reflect_episodes(episodes_json: str = None, **kw) -> dict:
            ...
            await self._notify("自己内省中...「本当にこのエピソード群でいいのか？」")

            reflect_result = await call_llm(
                tier="sonnet",
                system_prompt=EPISODE_SELF_REFLECT_PROMPT,
                user_message=f"以下のキャラクターとエピソード群について、本当にこれでいいか正直に自問してください:\n\n--- キャラクター ---\n{context}\n\n--- エピソード ---\n{json.dumps(data, ensure_ascii=False, indent=2)}",
                json_mode=True,
                cache_system=True,
            )
            ...
            if convinced:
                ...
                return {"status": "SUCCESS", "message": "SELF-REFLECT PASSED. submit_final_episodesで提出してください。"}
            else:
                self_reflect_convinced = False
                critique_passed = False
                ...
                return {"status": "FAILED", "message": f"まだ確信が持てません。理由: {reason}。改善案: {suggestion}。ドラフトを改善し、再度draft_episodes → request_critique → self_reflectの順で進めてください。"}
        ...
        tools = [
            AgentTool(
                name="draft_episodes",
                description="5-8個のエピソードのドラフトを提出し、構造バリデーションを受けます。新しいドラフトを提出するとcritique/self_reflectの結果はリセットされます。",
                input_schema={
                    "type": "object",
                    "properties": {
                        "episodes_json": {"type": "string", "description": "エピソードデータ全体のJSON文字列（{\"episodes\": [...]}形式）"}
                    },
                    "required": ["episodes_json"]
                },
                handler=draft_episodes
            ),
            AgentTool(
                name="request_critique",
                description="現在のエピソードドラフトを別のLLMが厳しく評価します。McAdams分布、Redemption Bias、具体性、感情深度等をチェックします。",
                input_schema={
                    "type": "object",
                    "properties": {
                        "episodes_json": {"type": "string", "description": "エピソードデータ全体のJSON文字列"}
                    },
                    "required": ["episodes_json"]
                },
            ...
            AgentTool(
                name="self_reflect",
                description="request_critiqueでpassを得た後に呼び出す内省ツール。「本当にこのエピソード群でいいのか？」を自問します。確信が持てない場合はcritique_passedもリセットされます。",
                input_schema={
                    "type": "object",
                    "properties": {
                        "episodes_json": {"type": "string", "description": "エピソードデータ全体のJSON文字列"}
                ...
                handler=self_reflect_episodes
            ),
            AgentTool(
                name="submit_final_episodes",
                description="request_critiqueでpass かつ self_reflectでconvinced=trueを得た後にのみ呼び出せます。最終エピソードデータをシステムに提出します。",
                input_schema={
                    "type": "object",
                    "properties": {
                        "episodes_json": {"type": "string", "description": "完成したエピソードデータ全体のJSON文字列"}
                    },
                    "required": ["episodes_json"]
                },
                handler=submit_final_episodes
            ),
        ]
...
【Phase 1: 計画】
- キャラクターのコンテキストを注意深く読む
- McAdamsカテゴリの分布を事前に計画する（redemption≤2, contamination≥1, loss≥1, ambivalent≥1, dream_origin=1）
- どの時期にどのカテゴリを配置するか決める

【Phase 2: ドラフト → 外部批評】
- draft_episodesツールで全エピソードのドラフトを提出する
- request_critiqueで品質評価を受ける
- refineの場合は指摘に基づいて改善し、再度draft_episodes → request_critique

【Phase 3: 自己内省 — 「本当にこれでいいのか？」】
- request_critiqueでpassを得たら、self_reflectツールを呼び出す
- 「このエピソード群は本当に面白いか？感情移入できるか？妥協していないか？」を正直に自問
- convinced=falseが返った場合: critique_passedもリセット。改善してPhase 2からやり直す

【Phase 4: 確信を持って提出】
- critique_passed=True かつ self_reflect_convinced=True でのみ
- submit_final_episodesツールで提出する
"""
        ...
        # ── フォールバック: 従来の2ステップ一発出し ──
        if not final_episodes_data:
            logger.warning("[A-3] Agentic loop failed. Falling back to one-shot pipeline.")
