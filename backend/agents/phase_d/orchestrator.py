...
class PhaseDOrchestrator:
    ...
    async def run(self) -> WeeklyEventsStore:
        ...
        if self.ws:
            await self.ws.send_agent_thought("[Phase D] WorldContext", f"世界設定生成完了", "complete")
            await self.ws.send_agent_thought("[Phase D] SupportingCharacters", f"周囲人物設計完了", "complete")
